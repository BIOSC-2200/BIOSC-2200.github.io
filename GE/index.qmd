...
#| fig-height: 4
#| fig-align: center
#| fig-cap: Frequency of mice exhibiting the small muscle phenotype in three lines aross the first 22 generations of selection. Data from Garland et al. [-@Garland2002-it]
...
We can use what we have learned to test whether it is likely that these breeding lines deviate from HWE. If so, it would suggest that the allele for the small-muscle phenotype was being selected for in the Selected lines (red lines in @fig-mmgen).
...
We can then repeat these calculations for Generation 22, using the observed count of 19 mice at that later generation. We will do this all in one code block.
...
We now have the expected counts from generation 14 that we would expect to find at generation 22 if the population followed HWE.
...
Note here that the generation 14 counts are the "Expected" counts at generation 22. The "Observed" counts are what we find in reality.

> At this point, we have what we need What is your prediction
